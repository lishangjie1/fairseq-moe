...
def save_checkpoint(
    ...
    delete_old_checkpoint_files(cfg, end_of_epoch, trainer.is_moe or trainer.is_base_moe, suffix, trainer.is_data_parallel_master, trainer.is_fsdp)


def delete_old_checkpoint_files(cfg: DictConfig, end_of_epoch: bool, is_moe: bool, suffix: str, is_data_parallel_master: bool, is_fsdp: bool):
    if not end_of_epoch and cfg.keep_interval_updates > 0:
        suffixes = [suffix]
        if is_moe:
            if not is_fsdp and is_data_parallel_master:
                suffixes.append("-shared")
            elif is_fsdp:
                suffixes.append(f'-shared-shard{dist_utils.get_global_rank()}')
    ...
    for filename in filenames:
        ...
        for shard_idx in range(num_shards):
            ...
            if (
                ...
                and cfg.model.langs is not None
