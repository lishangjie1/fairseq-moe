...
def fsdp_wrap_expert(args, layer, min_num_params=0):
    ...
    for p in layer.moe_layer.experts.parameters():
        p.expert = True, # some parameters may need to share parameters
...
class TransformerEncoder(FairseqEncoder):
    ...
    def collect_expert_choices(self, src_tokens):
        ...
        if not hasattr(self, 'src_token_to_expert'):
            self.src_token_to_expert=torch.zeros(enc_moe_layer_num, src_vocab_size, num_expert, device=src_tokens.device)
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        # embed_tokens_in_dict=state_dict['{}.embed_tokens.weight'.format(name)]
        # if self.embed_tokens.weight.shape!=embed_tokens_in_dict.shape:
        #     # currently, we only handel the case of adding new langugae pairs in checkpoint
        #     # new language pairs, but not new languages !!!
        #     assert self.embed_tokens.weight.shape[-1]==embed_tokens_in_dict.shape[-1]
        #     assert self.embed_tokens.weight.shape[0]>embed_tokens_in_dict.shape[0]
        #     pad_num=self.embed_tokens.weight.shape[0]-embed_tokens_in_dict.shape[0]
        #     embed_tokens_in_dict.data=torch.cat([embed_tokens_in_dict, self.embed_tokens.weight[-pad_num:].cpu()])
        #     assert self.embed_tokens.weight.shape[0]==embed_tokens_in_dict.shape[0]
        #     state_dict['{}.embed_tokens.weight'.format(name)]=embed_tokens_in_dict
        return state_dict
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def collect_expert_choices(self, tgt_tokens):
        ...
        if not hasattr(self, 'tgt_token_to_expert'):
            self.tgt_token_to_expert=torch.zeros(dec_moe_layer_num, tgt_vocab_size, num_expert, device=tgt_tokens.device)
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        # embed_tokens_in_dict=state_dict['{}.embed_tokens.weight'.format(name)]
        # output_projection_in_dict=state_dict['{}.output_projection.weight'.format(name)]
        # if self.embed_tokens.weight.shape!=embed_tokens_in_dict.shape:
        #     # currently, we only handel the case of adding new langugae pairs in checkpoint
        #     # new language pairs, but not new languages !!!
        #     assert self.embed_tokens.weight.shape[-1]==embed_tokens_in_dict.shape[-1]
        ...
        #     assert self.embed_tokens.weight.shape[0]==embed_tokens_in_dict.shape[0]
        #     state_dict['{}.embed_tokens.weight'.format(name)]=embed_tokens_in_dict
        # if self.output_projection.weight.shape!=output_projection_in_dict.shape:
        #     # currently, we only handel the case of adding new langugae pairs in checkpoint
        #     # new language pairs, but not new languages !!!
        #     assert self.output_projection.weight.shape[-1]==output_projection_in_dict.shape[-1]
        #     assert self.output_projection.weight.shape[0]>output_projection_in_dict.shape[0]
        #     pad_num=self.output_projection.weight.shape[0]-output_projection_in_dict.shape[0]
        #     output_projection_in_dict.data=torch.cat([output_projection_in_dict, self.output_projection.weight[-pad_num:].cpu()])
        #     assert self.output_projection.weight.shape[0]==output_projection_in_dict.shape[0]
        #     state_dict['{}.output_projection.weight'.format(name)]=output_projection_in_dict
        return state_dict
