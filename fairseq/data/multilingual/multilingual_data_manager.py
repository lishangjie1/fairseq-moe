...
from fairseq.utils import FileContentsAction, csv_str_list, eval_str_dict, print_r0
...
class MultilingualDatasetManager(object):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            default=None,
            type=str,
            choices=[EncoderLangtok.src.value, EncoderLangtok.tgt.value, EncoderLangtok.lang_pair.value],
            metavar="SRCTGT",
    ...
    def prepare(cls, load_dictionary, args, **kargs):
        ...
        def load_dictionary_and_postproc(path):
            ...
            if src_langtok_spec=='lang_pair' or tgt_langtok_spec=='lang_pair':
                # extra_lang_pairs = (
                #     list(
                #         {p for _, v in args.extra_lang_pairs.items() for p in v.split(",")}
                #     )
                ...
                #     else []
                # )
                # all_lang_pairs=sorted(args.lang_pairs+extra_lang_pairs)
                augment_dictionary(
                    dictionary=d,
                    language_list=args.lang_pairs,
                    lang_tok_style=args.lang_tok_style,
                    langtoks_specs=args.langtoks_specs,
                    extra_data=args.extra_data,
                )
    ...
    def get_encoder_langtok(self, src_lang, tgt_lang, spec=None):
        if spec is None:
            return None
        if spec and spec.startswith("lang_pair"):
            langtok='__{}-{}__'.format(src_lang, tgt_lang)
        elif spec and spec.startswith("src"):
            if src_lang is None:
                return None
    ...
    def load_split_datasets(
        ...
        datasets=[]
        for param in data_param_list:
            src, tgt=param['src'], param['tgt']
            indexed_dataset.hash_epoch_and_language_pair_to_seed(epoch, src, tgt)
            dataset=(
                param["key"],
                self.load_a_dataset(
                    combine=combine,
